...
export const useUserAchievements = (userId?: string) => {
  return useQuery({
    ...
    queryFn: async (): Promise<UserAchievement[]> => {
      const response = await apiClient.get<ApiSuccessResponse<UserAchievement[]>>(
        apiEndpoints.achievements.user,
...
export const useUnlockAchievement = () => {
  ...
  return useMutation({
    mutationFn: async (achievementId: string): Promise<UserAchievement> => {
      const response = await apiClient.post<ApiSuccessResponse<UserAchievement>>(
        apiEndpoints.achievements.unlock,
        { achievementId }
      );
      return response.data;
    },
